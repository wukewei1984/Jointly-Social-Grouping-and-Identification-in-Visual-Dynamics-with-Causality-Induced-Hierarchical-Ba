...
class VolleyballDataset(data.Dataset):
    ...
    """
    def __init__(self,anns,tracks,frames,images_path,image_size,feature_size,num_boxes=12,num_before=5,num_after=4,is_training=True,is_finetune=False):
        self.anns=anns
        self.tracks=tracks
        self.frames=frames
        ...
        if self.is_finetune:
            ...
            if self.is_training:
                ...
                return [(sid, src_fid, fid)
                        ...
                        for fid in[src_fid -4, src_fid - 3, src_fid-2, src_fid-1, src_fid , src_fid + 1, src_fid + 2,
                         src_fid +3, src_fid + 4, src_fid + 5]]
            else:
                return [(sid, src_fid, fid) 
                        for fid in  [src_fid -4, src_fid - 3, src_fid-2, src_fid-1, src_fid , src_fid + 1, src_fid + 2,
                         src_fid +3, src_fid + 4, src_fid + 5]]
