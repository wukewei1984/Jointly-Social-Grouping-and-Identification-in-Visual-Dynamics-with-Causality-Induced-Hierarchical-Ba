...
import numpy as np
import statsmodels.tsa.stattools as gct
...
class GCNnet_volleyball(nn.Module):
    ...
    def forward(self, batch_data):
        ...
        # print(boxes_features.shape)
        b = boxes_features.shape[0]
        c = boxes_features.shape[3]
        # 此处添加granger 模块
        # --------------------------------------11-02尝试下不挑选box而将全部的box进行shift
        ndarray = np.array(boxes_features)
        fold = c // 8

        out = torch.zeros_like(boxes_features)
        ...
        for k in range(b):
            ...
            for i in range(12):
                ...
                for j in range(12):
                    ...
                    maxValue = 0
                    indexCnt = 0
                    a1=np.sum(ndarray[k,:,i,:],axis=1).reshape(10,1)
                    b1 = np.sum(ndarray[k, :, j, :],axis=1).reshape(10, 1)
                    a1[1:, :] = a1[1:, :] - a1[:-1, :]
                    # print(np.mean(a1))
                    a1 - np.mean(a1)
                    b1[1:, :] = b1[1:, :] - b1[:-1, :]
                    ...
                    # print(c1)
                    dictgct = gct.grangercausalitytests(c1, 1, addconst=True, verbose=False)
                    if (dictgct[1][0]['ssr_ftest'][1] < maxP):
                        # print({}+"-"+{}+':'+dictgct)
                        if(index%11==0):print("------------------------------------")
                        index+=1
                        print("{}              {}".format(dictgct[1][0]['ssr_ftest'][0],dictgct[1][0]['ssr_ftest'][1]))
                        subNeedToShift=j




                # 此处将每个人应该融合的人的索引填入了needtoshift
                needtoshift.append(subNeedToShift)

            break

            # print("set={}".format(set1))
            # print("k={}".format(k))
            for index,box_id in enumerate(needtoshift):
                boxes_features[k,:-1,index,:fold]=boxes_features[k,1:,box_id,:fold]


        # # 此处为shift全部box的修改
        out = torch.zeros_like(boxes_features)
        # fold = c // 8
        out[:, :-1, :, :fold] = boxes_features[:, 1:, :, :fold]
        out[:, 1:, :, fold:2 * fold] = boxes_features[:, :-1, :, fold:2 * fold]
        out[:, :, :, 2 * fold:] = boxes_features[:, :, :, 2 * fold:]
